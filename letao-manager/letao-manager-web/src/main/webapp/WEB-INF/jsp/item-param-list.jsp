        ...
        handler:function(){
        	...
        	TAOTAO.createWindow({
        		...
        		onLoad:function(){
              	...
              	i:$.post("/item/param/reload/"+ids,function(data){
            		var paramStr=data.data;
            		var paramDatas=JSON.parse(paramStr);
            		//$.messager.alert('提示',JSON.stringify(paramDatas));
            		//var paramDatas=$.parseJSON(paramStr);
            		//$.messager.alert('提示',paramStr);
            		//var paramData;
            		
                  	//$.messager.alert('提示',JSON.stringify(paramDatas));
                		for(x in paramDatas){
            				//$.messager.alert('提示',paramData.group);
            				var	paramData=paramDatas[x];
            				var group=paramData.group;
            				//$.messager.alert('提示',group+"..."+x)
            				var j=$(".selectItemCat");
            				j.show();
            				j.text("1111");
            		} 
